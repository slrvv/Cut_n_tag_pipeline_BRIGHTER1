...
for(hist in sampleList){
  spikeRes = read.table(paste0(projPath, "/alignment/sam/bowtie2_summary/", hist, "_bowtie2_spikeIn_AmpR.txt"), header = FALSE, fill = TRUE)
  spikeDepth = read.table(paste0(projPath, "/alignment/sam/bowtie2_summary/", hist, "_bowtie2_spikeIn_AmpR.seqDepth"), header = FALSE, fill = TRUE)
  alignRate = as.numeric(spikeDepth$V1[1])/as.numeric(spikeRes$V1[1])
  ...
  spikeAlign = data.frame(Histone = histInfo[1], Replicate = paste(histInfo[-1], collapse = "_"), 
                          ...
                          MappedFragNum_spikeIn = spikeDepth$V1[1], 
...
write.table(spikeAlign, paste0(projPath,
                                "/alignment/summary_seq_depth_spikein_AmpR.txt"),
            row.names = FALSE)
